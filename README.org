...
With bookmarks+, you add bookmarks to websites with the src_sh[:exports code]{C-x x c u} keybinding. You can add bookmarks to directories by entering C-x d and with the directory name selected in the dired buffer, enter C-x x m to add the directory as a bookmark. 
